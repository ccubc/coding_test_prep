...
class Solution:
    def filterRestaurants(self, restaurants: List[List[int]], veganFriendly: int, maxPrice: int, maxDistance: int) -> List[int]:
        dic_results = collections.defaultdict(list)
        for r in restaurants:
            if (not veganFriendly) or r[2]==1:
                if r[3] <= maxPrice and r[4] <= maxDistance:
                    dic_results[r[1]].append(r[0])
        for k, v in dic_results.items():
            v.sort()
        ans = []
        for k in sorted(dic_results.keys())[::-1]:
            ans += dic_results[k][::-1]
        return ans
    ...
    def filterRestaurants(self, restaurants: List[List[int]], veganFriendly: int, maxPrice: int, maxDistance: int) -> List[int]:
        ...
        for i in range(len(restaurants)):
            ...
            if restaurants[i][3] > maxPrice or restaurants[i][4] > maxDistance:
                continue
            filter.append(restaurants[i])
        return [r[0] for r in sorted(filter, key = lambda x: (-x[1], -x[0]))]
...
class Solution(object):
    def filterRestaurants(self, restaurants, veganFriendly, maxPrice, maxDistance):
        ...
        return [i[0] for i in filtered_list]
